---
title: "【4. AWSデプロイ編】backendのデプロイ準備｜【独学ポートフォリオ開発応援】実務未経験から学べる！Rails×Next.js×AWSハンズオン解説"
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/aws-11"
author:
published:
created: 2026-04-29
description:
...
> 📍 **デプロイ全体フロー（現在地）**  
> ① VPC → ② サブネット → ③ IGW → ④ ルートテーブル → ⑤ セキュリティグループ → ⑥ RDS → ⑦ ECR → **⑧ コード準備(backend)** → ⑨ タスク定義 → ⑩ ALB → ⑪ サービス起動 → ⑫ CD自動化

### なぜコード修正が必要か

開発環境のコードをそのまま本番にデプロイしても動きません。本番環境では**nginxとの連携・本番DB接続・マスターキーの管理**など、開発環境と異なる設定が必要です。この章ではそれらを本番環境用のファイルとして整備します。
...
開発環境においては、railsコンテナ内で `$ rails s` コマンドを実行することで、 [http://localhost:3000](http://localhost:3000/) に対してアクセスをできるようになっていました。
...
![](https://static.zenn.studio/user-upload/2766473e7b72-20230817.png)
...
- [ ] `nginx/Dockerfile.prod` と `nginx/nginx.conf` が作成されている
- [ ] `rails/Dockerfile.prod` が作成されている
- [ ] `rails/entrypoint.prod.sh` が作成されている（`db:create → db:migrate → db:seed → pumactl start` の順）
- [ ] `rails/config/puma.rb` がソケット通信用に書き換えられている
- [ ] `rails/config/credentials.yml.enc` に本番DBのURLが暗号化保存されている
- [ ] `rails/config/database.yml` に `production:` セクションが追加されている
- [ ] `rails/config/settings/production.yml` に本番フロントエンドドメインが設定されている
- [ ] `master.key` は `.gitignore` に含まれており **git管理外** になっている（⚠️ コミットに含めないこと）