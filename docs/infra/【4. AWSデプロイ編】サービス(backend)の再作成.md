...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/aws-18"
author:
published:
created: 2026-04-29
description:
...
> 📍 **デプロイ全体フロー（現在地）**  
> ① VPC → ... → ⑩ ALB(backend) → **⑪ サービス再作成+Route53設定(backend)** → ⑫ CD自動化

### なぜサービスを一度削除して再作成するか

ALBの設定はサービス作成時にのみ行えます。既存サービスにALBを後から追加することはできないため、いったん削除して再作成します。
...
![](https://static.zenn.studio/user-upload/0842f6dd6f7f-20230818.png)
...
![](https://static.zenn.studio/user-upload/f59013bb57ae-20230818.png)

![](https://static.zenn.studio/user-upload/5c019b672049-20230818.png)
...
![](https://static.zenn.studio/user-upload/c3872cd95ad3-20230818.png)

↓

ALBを経由してアプリケーションにアクセスできることを確認します。ALB(backend)の詳細画面に入りに、 **DNS名** を確認してください。

![](https://static.zenn.studio/user-upload/32d1ca37d268-20230818.png)

ブラウザから `http://{{ ALB DNS名 }}/api/v1/health_check` にアクセスをし、ヘルスチェックメッセージが返ってくればOKです！

![](https://static.zenn.studio/user-upload/cab1eedac2ab-20230818.png)
...
![](https://static.zenn.studio/user-upload/0b8b392fd9e8-20230818.png)

↓

「レコードを作成」をクリックし、レコードが正常に作成されればOKです。

![](https://static.zenn.studio/user-upload/1abd8f686853-20230818.png)
...
![](https://static.zenn.studio/user-upload/a50e079cb83a-20230818.png)

## ✅ この章の完了確認

- [ ] サービス `zenn-clone-backend-service` が再作成され、ロードバランシングにALBが設定されている
- [ ] Route53に `backend.ドメイン名` のAレコードが作成されている（ALBへのエイリアス）
- [ ] `https://backend.{{ 独自ドメイン }}/api/v1/health_check` でヘルスチェックメッセージが返ってくる（HTTPS接続）