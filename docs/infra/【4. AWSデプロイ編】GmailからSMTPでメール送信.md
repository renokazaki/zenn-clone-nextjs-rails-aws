...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/aws-26"
author:
published:
created: 2026-04-29
description:
...
> 📍 **この章の位置づけ**  
> メインのデプロイフロー12ステップは完了済みです。この章は**サインアップ機能を本番で動かすための追加設定**です。

### なぜGmailのSMTPを使うか

本番環境では deviseがメール認証を要求するため、メール送信インフラが必要です。SendGridなどの専用サービスもありますが、Gmailのアプリパスワードを使ったSMTP送信は**無料・設定が簡単**でポートフォリオ用途に最適です。
...
![](https://static.zenn.studio/user-upload/f8b5ede8d8ef-20230603.png)

「Google にログインする方法 > 2 段階認証プロセス」から、任意の手段で2段階認証プロセスを有効にします（すでに有効にしている場合はスキップ）。

![](https://static.zenn.studio/user-upload/976b7eab1518-20230603.png)

↓

![](https://static.zenn.studio/user-upload/136a1b9fecdc-20230603.png)

↓

「2 段階認証プロセス」ページの画面最下部にある「アプリ パスワード」に入ります。

![](https://static.zenn.studio/user-upload/2d4210bd4500-20230603.png)
...
![](https://static.zenn.studio/user-upload/2814f6f22173-20230603.png)

![](https://static.zenn.studio/user-upload/444e311d114c-20230603.png)

↓

英数字16字のパスワードが生成されるので、画面を閉じる前に忘れずにメモをしておきます。

![](https://static.zenn.studio/user-upload/8f09b9d60682-20230603.png)
...
![](https://static.zenn.studio/user-upload/49631d7d5b0a-20230823.png)

↓

「送信する」ボタンをクリックすると、メールアドレス宛に認証メールが送信されます。「アカウントを有効化する」をクリックしてください。

![](https://static.zenn.studio/user-upload/08c2a8a3f496-20230823.png)

↓

アカウントの有効化が完了し、サインインページにリダイレクトされますので、登録したメールアドレス、パスワードでサインインできることを確認してください。

![](https://static.zenn.studio/user-upload/7b0177df8f26-20230823.png)

## ✅ この章の完了確認

- [ ] Googleアカウントのアプリパスワード（16文字）が発行されている
- [ ] `credentials` に `production.gmail.user_name` と `production.gmail.password` が保存されている
- [ ] `rails/config/environments/production.rb` にSMTP設定が追記されている
- [ ] Railsイメージを再ECRプッシュしてbackendタスクを再起動した
- [ ] 本番環境でサインアップを実行し、認証メールが届いてアカウントが有効化できる