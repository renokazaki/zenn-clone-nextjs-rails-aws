...
> 📍 **この章の位置づけ**  
> CDパイプライン（⑫）が未設定の間、コード変更を本番に反映するには**手動での再デプロイ**が必要です。CDが設定された後はGitHub ActionsがこのECR再プッシュ＋タスク再起動を自動で行います。

> 💡 **なぜタスクを停止させると再起動するか**  
> サービスには「必要なタスク数 = 1」が設定されています。タスクを手動で停止すると、サービスが「タスク数が0になった」と検知して自動的に新しいタスクを起動します。この仕組みを利用して、ECRの最新イメージを使った新しいタスクに切り替えます。
...
- [ ] ECRの `zenn-clone-rails` リポジトリに新しいイメージがプッシュされている（プッシュ日時が最新）
- [ ] 旧タスクが停止され、新しいタスクが起動している
- [ ] 新タスクの `rails` コンテナと `nginx` コンテナが「HEALTHY」になっている
- [ ] ヘルスチェックURLにアクセスしてコード変更が反映されていることを確認できる
- [ ] `entrypoint.prod.sh` から `db:create` と `db:seed` がコメントアウトされている