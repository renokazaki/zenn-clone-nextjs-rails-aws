...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/aws-25"
author:
published:
created: 2026-04-29
description:
...
> 📍 **デプロイ全体フロー（現在地）**  
> ① VPC → ... → ⑩ ALB(frontend) → **⑪ サービス再作成+Route53設定(frontend)** → ⑫ CD自動化

### この章で完成すること

フロントエンドに独自ドメインが付与され、`https://{{ 独自ドメイン }}` からアプリにアクセスできるようになります。サインアップ以外の全機能が動作する状態になります。

## 手順

「ECS > クラスター > zenn-clone-cluster」にアクセスし、稼働している `zenn-clone-frontend-service` サービスをいったん削除します。

![](https://static.zenn.studio/user-upload/c1d720e5a74e-20230821.png)
...
![](https://static.zenn.studio/user-upload/dc45f54fc75d-20230822.png)

![](https://static.zenn.studio/user-upload/7c42e44a30d4-20230822.png)  
↓
...
![](https://static.zenn.studio/user-upload/008d2abdf57a-20230822.png)

↓

ALBを経由してアプリケーションにアクセスできることを確認します。ALB(frontend)の詳細画面に入りに、 **DNS名** を確認してください。

![](https://static.zenn.studio/user-upload/a4188add1d8c-20230822.png)

ブラウザから `http://{{ ALB DNS名 }}/api/health_check` にアクセスをし、ヘルスチェックメッセージが返ってくればOKです！

![](https://static.zenn.studio/user-upload/dfb690576d5a-20230822.png)
...
![](https://static.zenn.studio/user-upload/815585fbab36-20230818.png)

↓

もし、「レコードをクイックに作成」の画面が表示されたら、右上の「ウィザードに切り替える」をクリックして画面を切り替えてください。

![](https://static.zenn.studio/user-upload/b4eb784e112d-20230818.png)

↓

ルーティングポリシーを選択する画面が表示されたら、「シンプルルーティング」を選択して、「次へ」を押してください。

![](https://static.zenn.studio/user-upload/19f0a6937707-20230818.png)
...
![](https://static.zenn.studio/user-upload/b190838dbba0-20230822.png)
...
![](https://static.zenn.studio/user-upload/aac663eea68d-20230822.png)
...
![](https://static.zenn.studio/user-upload/b13d21f15297-20230908.png)

↓

「 `HTTP:80` 」にチェックを入れ、「リスナーの管理 > リスナーの編集」でリスナーの編集画面に入ってください。

![](https://static.zenn.studio/user-upload/632398293ee8-20230908.png)
...
![](https://static.zenn.studio/user-upload/42177f0228cb-20230908.png)

↓

動作確認をしてみます。 `http://{{ 独自ドメイン }}` でアクセスをすると、 `https://{{ 独自ドメイン }}` の方にリダイレクトされるようになっているはずです。

![](https://static.zenn.studio/user-upload/6f84d79b904e-20230908.png)

## ✅ この章の完了確認

- [ ] サービス `zenn-clone-frontend-service` が再作成され、ALB(frontend)が設定されている
- [ ] Route53に `ドメイン名`（レコード名空欄）のAレコードが作成されている（ALBへのエイリアス）
- [ ] `https://{{ 独自ドメイン }}` でトップページにアクセスでき、記事一覧が表示される
- [ ] HTTPでアクセスすると自動的にHTTPSにリダイレクトされる（301リダイレクト）
- [ ] サインアップ以外の機能（記事閲覧・サインイン・記事投稿）が正常に動作する