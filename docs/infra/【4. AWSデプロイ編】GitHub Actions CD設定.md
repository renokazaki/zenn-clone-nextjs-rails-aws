...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/aws-27"
author:
published:
created: 2026-04-29
description:
...
現状はコード変更のたびに「手動でDockerビルド → ECRプッシュ → タスク再起動」を行う必要があります。CDを設定すると**PRをmainにマージするだけで自動デプロイ**が走ります。また、CIチェック（rspec・rubocop・eslint）がパスしないとデプロイが止まるため、**壊れたコードが本番に出ることを防げます**。

#### 参考記事

[https://dev.classmethod.jp/articles/GitHub](https://dev.classmethod.jp/articles/GitHub) Actionss-ecs-fargate-cicd-beginner/
...
![](https://static.zenn.studio/user-upload/2b90b036695f-20230823.png)

ユーザー名を任意（ここでは「GitHub ActionsUser」）に設定し、「次へ」を押してください。

![](https://static.zenn.studio/user-upload/f275bcecaba3-20230823.png)
...
> 💡 **なぜこの2つのポリシーか**  
> CDパイプラインがやることは「ECRへのpush」と「ECSタスクの更新・再起動」の2つだけです。それ以上の権限（EC2の作成・削除など）を付与するのは最小権限の原則に反します。このIAMユーザーはCD専用なので、必要最小限の権限だけを付与します。

![](https://static.zenn.studio/user-upload/18f83a48db1f-20230823.png)

![](https://static.zenn.studio/user-upload/41600a910f72-20230823.png)

↓

ポリシーを選択できたら「次へ」進み、確認画面で「ユーザーの作成」を押してユーザーを作成してください。

![](https://static.zenn.studio/user-upload/d95befb8cfb9-20230823.png)

↓

ユーザーが正常に作成できたら、該当ユーザーの詳細画面に入り、タブ「セキュリティ認証情報」から、「アクセスキーを作成」ボタンをクリックしてください。

![](https://static.zenn.studio/user-upload/e44df16b3fd2-20230826.png)

![](https://static.zenn.studio/user-upload/0289e678d246-20230823.png)

↓

「主要なベストプラクティスと代替案にアクセスする」で「その他」を選択して、「次へ」進んでください。

![](https://static.zenn.studio/user-upload/d7251eafae5a-20230823.png)

説明タグを任意で設定し（空でもOK）、「アクセスキーを作成」を押してください。

![](https://static.zenn.studio/user-upload/d87fe68235ab-20230823.png)

↓

**アクセスキー** と **シークレットアクセスキー** が表示されるので、画面を閉じる前にどこかにメモしておいてください。

![](https://static.zenn.studio/user-upload/582b3d5a957b-20230823.png)
...
![](https://static.zenn.studio/user-upload/01cc53dbe021-20230823.png)
...
| Name | Secret |
| --- | --- |
| AWS\_ACCESS\_KEY\_ID | アクセスキーを入力 |
| AWS\_SECRET\_ACCESS\_KEY | シークレットアクセスキーを入力 |

> ⚠️ **アクセスキーの管理**：アクセスキーとシークレットアクセスキーは**絶対にコードにハードコードしないでください**。GitHub Secretsに保存することで、ワークフロー実行時だけ参照でき、外部からは見えない状態になります。万一流出した場合はIAMコンソールから即座に無効化してください。

![](https://static.zenn.studio/user-upload/4ce1892689a8-20230827.png)

↓

**AWS\_ACCESS\_KEY\_ID** 、 **AWS\_SECRET\_ACCESS\_KEY** が保存されればOKです！

![](https://static.zenn.studio/user-upload/f5d8cee950f8-20230827.png)

![](https://static.zenn.studio/user-upload/b796b672915c-20230823.png)
...
![](https://static.zenn.studio/user-upload/9b32ba8611f7-20230827.png)
...
![](https://static.zenn.studio/user-upload/f757c2b80b49-20230827.png)

↓

同じ要領で、 `zenn-clone-task-definition-frontend` の最新リビジョンにおけるタスク定義のJSON化コードを取得し、 `next/task-definition.json` にコピペしてください。

![](https://static.zenn.studio/user-upload/4388a714a134-20230827.png)
...
二つ目は **workflow\_call** です。これは「他のワークフローから呼び出された時に動作させる」という趣旨の設定で、これによってCDワークフロー`.github/workflows/cd.yml` の方からこちらのワークフローを呼び出すことができるようになります。
...
![](https://static.zenn.studio/user-upload/c477c944480a-20230827.png)

↓

Pull request エディット画面が表示されます。マージ先が **main** であることを確認し、タイトルと本文を任意で入力して「Create pull request」してください。

![](https://static.zenn.studio/user-upload/d213d0ed55c0-20230827.png)
...
![](https://static.zenn.studio/user-upload/dceaa4dad495-20230827.png)

![](https://static.zenn.studio/user-upload/1f4d2be111fd-20230827.png)

↓

時間が経過すると、CIチェックが全て完了しますので、「Merge Pull Request > Confirm」ボタンを押してください。

![](https://static.zenn.studio/user-upload/fa512693ba39-20230827.png)
...
![](https://static.zenn.studio/user-upload/72fce96994c9-20230827.png)

![](https://static.zenn.studio/user-upload/597569646091-20230827.png)

↓

**ci** ジョブが全て完了すると、 **deploy-rails** ジョブ、 **deploy-next** ジョブが動作し始めます。もし **ci** ジョブがエラーになった場合は、これらデプロイジョブは開始されないようになっています。

![](https://static.zenn.studio/user-upload/99c0d9f3036b-20230827.png)

↓

時間が経過すると、 **deploy-rails** ジョブ、 **deploy-next** ジョブも完了します。

![](https://static.zenn.studio/user-upload/d5775e7ee74c-20230827.png)

Rails, Nextそれぞれについて本番環境のヘルスチェックパスにアクセスし、先ほどのメッセージ変更が反映されていればOKです！

![](https://static.zenn.studio/user-upload/c8a4032ea069-20230827.png)

![](https://static.zenn.studio/user-upload/e116441baff1-20230827.png)
...
ターミナル(リポジトリの最新状況をローカルに取得)

```
git fetch
```

ターミナル(リポジトリの最新のmainブランチをローカルのmainブランチのマージ)
...
| 症状 | 原因 | 対処 |
| --- | --- | --- |
| `cd.yml` のCIジョブが動かない | `ci.yml` に `workflow_call:` が追加されていない | `ci.yml` の `on:` セクションに `workflow_call:` を追加する |
| mainへのPRマージ後もCDが走らない | `cd.yml` の `on.push.branches` が `main` になっていない、またはファイルのインデントがずれている | YAMLのインデントを確認し、`branches: ["main"]` が正しく設定されているか確認 |
| `Error: Could not assume role` | `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` がSecretに正しく設定されていない | GitHub Secrets の値をIAMで再発行したアクセスキーで上書きする |
| ECRへのプッシュは成功するがECSデプロイで失敗 | `task-definition.json` の内容が古いリビジョンのもの | AWSコンソールから最新タスク定義のJSONを再取得して `task-definition.json` を更新する |
| `ci` ジョブが失敗してデプロイが止まる | テスト（rspec・rubocop・eslint）がパスしていない | CIのエラーメッセージを確認してコードを修正する。CDは「CIパスが前提」なので正しい動作 |

## ✅ この章の完了確認

- [ ] IAMユーザー（GitHub ActionsUser）が作成され、2つのポリシーが付与されている
- [ ] アクセスキーとシークレットアクセスキーがGitHub Secretsの `AWS_ACCESS_KEY_ID` と `AWS_SECRET_ACCESS_KEY` に保存されている
- [ ] `rails/task-definition.json` と `next/task-definition.json` がコミットに含まれている
- [ ] `.github/workflows/cd.yml` が実装されている
- [ ] `.github/workflows/ci.yml` に `branches-ignore: main` と `workflow_call:` が追加されている
- [ ] PRをmainにマージ後、Actionsタブで `deploy-rails` と `deploy-next` ジョブが**グリーン**で完了する
- [ ] 本番環境のヘルスチェックURLにアクセスしてメッセージ変更が反映されていることを確認できる