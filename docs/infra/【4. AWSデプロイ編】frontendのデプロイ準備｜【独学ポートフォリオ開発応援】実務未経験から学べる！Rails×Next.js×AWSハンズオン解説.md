---
title: "【4. AWSデプロイ編】frontendのデプロイ準備｜【独学ポートフォリオ開発応援】実務未経験から学べる！Rails×Next.js×AWSハンズオン解説"
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/aws-20"
author:
published:
created: 2026-04-29
description:
...
> 📍 **デプロイ全体フロー（現在地）**  
> ① VPC → ② サブネット → ③ IGW → ④ ルートテーブル → ⑤ セキュリティグループ(frontend) → ⑥ RDS → ⑦ ECR(frontend) → **⑧ コード準備(frontend)** → ⑨ タスク定義(frontend) → ⑩ ALB(frontend) → ⑪ サービス起動 → ⑫ CD自動化

### なぜコード修正が必要か

Next.jsも開発環境と本番環境で環境変数・ポート番号・Dockerfileが異なります。特に**80番ポートでの起動**と、**本番ドメインを指す環境変数**への切り替えが重要です。マルチステージビルドのDockerfileにより、本番イメージのサイズを最小化します。
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__8659171ab9be" frameborder="0"></iframe>
...
- [ ] `next/.env.production` が作成されている（`NEXT_PUBLIC_API_BASE_URL` と `NEXT_PUBLIC_FRONT_BASE_URL` が本番ドメインに設定）
- [ ] `next/package.json` の `start` スクリプトが `next start -p 80` になっている
- [ ] `next/Dockerfile.prod` がマルチステージビルドで作成されている（ビルドステージ + 実行ステージ）
- [ ] `.env.production` は `.gitignore` に含まれて**いない**（ビルド時にコンテナへコピーされる必要があるため）