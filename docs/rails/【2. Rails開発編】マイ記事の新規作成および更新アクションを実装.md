...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/rails-06"
author:
published:
created: 2026-04-29
description:
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__e5f57b7b99faf" frameborder="0"></iframe>
...
![](https://static.zenn.studio/user-upload/0f290cab305e-20230718.png)
...
次に、 `current_user.articles.unsaved.first` で、サインインユーザーが保有している未保存記事を取得します。これが `nil` でない場合は、該当の article レコードが unsaved\_article に代入されます。

`current_user.articles.unsaved.first` が `nil` であった場合は、論理和の後半である `current_user.articles.create(status: :unsaved)` が実行されて未保存記事が新規作成され、これが unsaved\_article に代入されます。
...
| --- | --- |
| METHOD | POST |
| URL | [http://localhost:3000/api/v1/current/articles](http://localhost:3000/api/v1/current/articles) |
| HEADERS | 下記参照 |
| BODY | 空欄 |
...
![](https://static.zenn.studio/user-upload/f8243c55aa4a-20230904.png)

![](https://static.zenn.studio/user-upload/1fdd313f466e-20230719.png)
...
![](https://static.zenn.studio/user-upload/3c1b74d85d13-20230719.png)
...
articles\_controller に udpdate アクションを追加します。
...
| --- | --- |
| METHOD | PATCH |
| URL | [http://localhost:3000/api/v1/current/articles/:id](http://localhost:3000/api/v1/current/articles/:id) |
| HEADERS | 下記参照 |
| BODY | 下記参照 |
...
![](https://static.zenn.studio/user-upload/4d146fdb9f5d-20230722.png)

![](https://static.zenn.studio/user-upload/1befd3602967-20230722.png)

Sequel Ace で該当の articles レコードが意図通り更新されていればOKです！

![](https://static.zenn.studio/user-upload/dafb6b717acf-20230722.png)
