...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/setup-10"
author:
published:
created: 2026-04-29
description:
...
> **[現バージョン対応メモ]**
>
> **App Router ではサーバーコンポーネントで直接 fetch できる（`useSWR` 不要）:**
>
> 教材では `useSWR` + `axios` の組み合わせでクライアントサイドフェッチを行っているが、App Router のサーバーコンポーネント（デフォルト）では `async/await` + `fetch()` でサーバー側からデータ取得できる。
>
> **サーバーコンポーネントでの fetch 例（教材のヘルスチェック相当）:**
>
> ```tsx
> // src/app/page.tsx（サーバーコンポーネント = デフォルト）
...
> **Docker 内での URL について:**
>
> サーバーコンポーネントのフェッチはサーバー（Next.js コンテナ）側から実行されるため、URL は `http://localhost:3000` ではなく **`http://rails:3000`**（Docker のサービス名）を使う。
> ブラウザから直接叩くクライアントコンポーネントの場合は引き続き `http://localhost:3000`。
>
> **`useSWR` を使う場面:**
>
> クライアントサイドでのリアルタイム更新や、ユーザー操作に応じた再フェッチが必要な場合は引き続き `useSWR` を使う。その場合は該当コンポーネントの先頭に `'use client'` が必要。
>
> **CORS 設定について:**
>
> サーバーコンポーネントからのフェッチはブラウザを経由しないため CORS は関係しない。クライアントコンポーネントから直接 Rails にリクエストを投げる場合のみ CORS 設定が必要。
...
しかし、この状態で [http://localhost:8000/](http://localhost:8000/) にアクセスしてみると、「An error has occurred.」が表示されているはずです。

エラーになっている理由は、RailsAPI側での **CORS設定** が行われておらず、Next.jsからのリクエストを許可していないためです。

![](https://static.zenn.studio/user-upload/7868ab6f8238-20230629.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__429b249bf9344" frameborder="0"></iframe>
...
railsサーバーが再起動したら、再び [http://localhost:8000](http://localhost:8000/) に戻り、画面をリロードしてください。「Success Health Check!」が表示されていればOKです！

![](https://static.zenn.studio/user-upload/b575d8dc7729-20230629.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__5342a49ded28b" frameborder="0"></iframe>
...
一度 Rails サーバーを再起動し、 [http://localhost:8000/](http://localhost:8000/) から再度アクセスして、疎通が問題なく行えていることを確認してください。
...
![](https://static.zenn.studio/user-upload/5e70e9ca1464-20230702.png)