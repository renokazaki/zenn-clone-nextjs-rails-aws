...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/next-06"
author:
published:
created: 2026-04-29
description:
...
> **[現バージョン対応メモ]**
>
> **`useSWR` を global state として使うパターンについて:**
>
> 教材では `useSWR` にキーを渡し `fetcher` を `null` にすることで「キャッシュに値を書き込む/読み出す」という独自の global state パターンを実装している。このパターンは動作するが、SWR 本来の用途（データフェッチ）から外れており、将来的に挙動が変わるリスクがある。
>
> **App Router での代替パターン（React Context）:**
>
> ```tsx
...
> const AuthContext = createContext<{ currentUser: User; setCurrentUser: (u: User) => void }>({
>   currentUser: null,
>   setCurrentUser: () => {},
> })
>
...
> ```
>
> `AuthProvider` を `src/app/layout.tsx` でラップし、各コンポーネントから `useAuth()` で取得する。
>
> **`<Auth>` コンポーネントの代替:**
>
> `_app.tsx` が存在しないため、`<Auth>` コンポーネントを置く場所は `src/app/layout.tsx` 内の Client Component として配置する。
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__3d35e359acebf" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__2ef43fdc2af6" frameborder="0"></iframe>
...
![](https://static.zenn.studio/user-upload/d084048d8846-20230806.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__ff2407d9cba56" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__5ce94d1a723b9" frameborder="0"></iframe>
...
いよいよ動作を確認してみます。 /sign\_in ページでのサインインを行なって localStorage には認証情報が保存されているとします。

![](https://static.zenn.studio/user-upload/b74b5040f58c-20230802.png)

この状態で画面をリロードすると、 Header コンポーネント右側にユーザー名が表示されることを確認できます。

![](https://static.zenn.studio/user-upload/1a40d75b98fa-20230802.png)
...
次に、未ログイン状態の挙動を確認します。Chromeの「検証 > Application > Storage > LocalStorage > [http://localhost:8000/](http://localhost:8000/) 」で認証情報を開き、直上のバツアイコンから localStorage の情報を削除できます。

![](https://static.zenn.studio/user-upload/c5348475c1bb-20230802.png)

![](https://static.zenn.studio/user-upload/9577e376e29a-20230802.png)

認証情報を削除してから画面をリロードしてください。再び、「SignIn」「SignUp」ボタンが表示されるようになるはずです。

![](https://static.zenn.studio/user-upload/7f9d82db2019-20230802.png)
...
ユーザー未ログインの状態で /sign\_in に入り、ユーザー認証を行ってみてください。なぜか、Header右側が「SignIn」「SignUp」ボタンのままになるはずです。

![](https://static.zenn.studio/user-upload/792886713bff-20230802.png)

その後、画面をリロードすると、正しくユーザー名が表示されるようになり、極めて不自然な挙動となっています。

![](https://static.zenn.studio/user-upload/2c3b33741a90-20230802.png)
...
![](https://static.zenn.studio/user-upload/bec7dcd58d1d-20230802.png)