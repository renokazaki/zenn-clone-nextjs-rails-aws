---
title: "【1. 環境構築編】Next.jsの仕組み｜【独学ポートフォリオ開発応援】実務未経験から学べる！Rails×Next.js×AWSハンズオン解説"
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/setup-08"
author:
published:
created: 2026-04-29
description:
...
> **[現バージョン対応メモ]**
>
> 教材は Pages Router ベースで解説しているが、App Router では仕組みが異なる。
>
> **ルーティングの違い:**
>
> | Pages Router（教材） | App Router（本プロジェクト） |
> |---------------------|--------------------------|
> | `src/pages/hello_world.tsx` → `/hello_world` | `src/app/hello_world/page.tsx` → `/hello_world` |
> | `src/pages/index.tsx` → `/` | `src/app/page.tsx` → `/` |
> | `src/pages/_app.tsx` で全ページラップ | `src/app/layout.tsx` で全ページラップ |
> | `src/pages/_document.tsx` でHTMLマークアップ | `src/app/layout.tsx` の `<html>` タグで管理 |
>
> **コンポーネント型の違い:**
>
> | Pages Router（教材） | App Router（本プロジェクト） |
> |---------------------|--------------------------|
> | `NextPage` 型（pages配下） | 型注釈なし（デフォルトがサーバーコンポーネント） |
> | `React.FC` 型（components配下） | `React.FC` 型（クライアントコンポーネントは先頭に `'use client'`） |
>
> **サーバーコンポーネント vs クライアントコンポーネント:**
>
> App Router ではすべてのコンポーネントはデフォルトで **サーバーコンポーネント（RSC）**。
> - `useState` / `useEffect` / イベントハンドラを使う場合 → ファイル先頭に `'use client'` を追加
> - RSC 内では `async/await` で直接 `fetch()` が使える（`useSWR` 不要）
...
[http://localhost:8000/hello\_world](http://localhost:8000/hello_world) にアクセスすると、「Hello World!」が画面に表示されていることが確認できると思います。

![](https://static.zenn.studio/user-upload/04fddbc46afb-20230627.png)
...
再度 [http://localhost:8000/hello\_world](http://localhost:8000/hello_world) を確認すると、変更が反映されていることが確認できるはずです。

![](https://static.zenn.studio/user-upload/fddb85669277-20230627.png)
...
再度 [http://localhost:8000/hello\_world](http://localhost:8000/hello_world) を確認すると、正常に画面が読み込まれていることが確認できるはずです。

![](https://static.zenn.studio/user-upload/f8976497b22e-20230627.png)
...
この状態で [http://localhost:8000/hello\_world](http://localhost:8000/hello_world) を確認すると、「Click Here」というボタンが画面内に配置されていることが確認できます。

![](https://static.zenn.studio/user-upload/5be4b8ca1330-20230627.png)
...
![](https://static.zenn.studio/user-upload/7e5c9004a33d-20230628.png)
...
![](https://static.zenn.studio/user-upload/1970f386f25e-20230628.png)
...
![](https://static.zenn.studio/user-upload/e6c2ea50ba36-20230628.png)
...
この状態で「From HelloWorld」をクリックすると、ログとして「Clicked from hello\_world」が出力されているはずです。

![](https://static.zenn.studio/user-upload/f622efd0fe2a-20230628.png)
...
これを前提にすると、「 **path: /hello\_world の画面** 」 と 「 **pages/hello\_world.tsxファイル** 」 が対応している原理が理解できるようになります。
