---
title: "【3. Next.js開発編】記事一覧ページのモックを実装｜【独学ポートフォリオ開発応援】実務未経験から学べる！Rails×Next.js×AWSハンズオン解説"
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/next-01"
author:
published:
created: 2026-04-29
description:
...
> **[現バージョン対応メモ]**
>
> **App Router ではページファイルのパスと構成が変わる:**
>
> | 教材（Pages Router） | App Router（本プロジェクト） |
> |--------------------|--------------------------|
> | `src/pages/index.tsx` | `src/app/page.tsx` |
> | `NextPage` 型を使用 | 型なし（サーバーコンポーネント） |
> | `useSWR` でクライアントフェッチ | `async/await` でサーバーフェッチ |
>
> **サーバーコンポーネントで書き直した場合の記事一覧ページ例:**
>
> ```tsx
> // src/app/page.tsx
...
> **`fetch` の URL について:**
>
> サーバーコンポーネントは Next.js コンテナ内で実行されるため `http://rails:3000`（Docker サービス名）を使う。
>
> **ローディング表示について:**
>
> サーバーコンポーネントでは `useSWR` のようなクライアントローディング状態がないが、`src/app/loading.tsx` を配置することで Next.js が自動的にローディング UI を表示する（Suspense ベース）。
...
![](https://static.zenn.studio/user-upload/15209e9882b5-20230724.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__53d57ff071e7b" frameborder="0" height="121.8125"></iframe>
...
![](https://static.zenn.studio/user-upload/8145be38731e-20230904.png)
...
トップページ [http://localhost:8000](http://localhost:8000/) にアクセスをしてください。もし何らかのエラー画面が表示されたら、一度サーバーの再起動を試してみてください。
...
![](https://static.zenn.studio/user-upload/259314c0f4eb-20230724.png)

### 実装内容の理解

現時点では、以下のような機能を持った記事一覧ページが実装されています。

- 公開中の articles レコード全体のうち、created\_atに対する降順で先頭10件の情報が表示されている（=1ページ目の画面が表示されている）
...
![](https://static.zenn.studio/user-upload/b1542943da3c-20230724.png)
...
![](https://static.zenn.studio/user-upload/de4484839507-20230723.gif)
...
![](https://static.zenn.studio/user-upload/ea89a6722ac9-20230723.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__fe311c3e319bb" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__946ee87315bae" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__e2d85d110f38a" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__092884ff3c15f" frameborder="0"></iframe>

今回は `md` にしているので、 `<Conatiner>` 要素の最大幅は900pxに設定されています。Chrome検証ツールからも確認することができます。

![](https://static.zenn.studio/user-upload/6c6e9a96a898-20230723.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__7ff7ef280e867" frameborder="0"></iframe>
...
![](https://static.zenn.studio/user-upload/30e266973b46-20230723.gif)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__da704d595e3bf" frameborder="0"></iframe>

実際の画面上で任意の記事カードをクリックすると、 `/articles/:id` にアクセスできることを確認できるはずです（当URLは後ほど記事詳細ページが実装される予定ですが、現状は未実装なので404エラーになります）

![](https://static.zenn.studio/user-upload/78b043841080-20230723.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__d59986e03cecc" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__8cd28906768af" frameborder="0"></iframe>
...
![](https://static.zenn.studio/user-upload/1912da36adda-20230723.png)

すると、画面上では45文字目までしか記事カードタイトルとして表示されず、それ以降は`...`という文字列に置き換わって表示されていることが確認できます。

![](https://static.zenn.studio/user-upload/0ba7496343df-20230724.png)
...
![](https://static.zenn.studio/user-upload/c873059bde79-20230724.png)
