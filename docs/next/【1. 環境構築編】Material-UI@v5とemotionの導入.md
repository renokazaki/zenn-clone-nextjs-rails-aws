...
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/setup-09"
author:
published:
created: 2026-04-29
description:
...
> **[現バージョン対応メモ]**
>
> **`_document.tsx` / `_app.tsx` は App Router では存在しない:**
>
> 教材では emotion の SSR 対応のために `_document.tsx` と `_app.tsx` に大量の設定コードを追加しているが、App Router では両ファイルが存在しないため、この設定方法は使えない。
>
> **App Router での MUI + emotion セットアップ:**
>
> 1. `src/app/layout.tsx` にテーマプロバイダーを配置する
> 2. emotion の SSR には `@emotion/cache` + `useServerInsertedHTML`（Next.js 提供）を使う
> 3. MUI 公式が App Router 向けのサンプルを提供している
>    → https://github.com/mui/material-ui/tree/master/examples/material-ui-nextjs-ts
>
> **App Router での最小セットアップ例:**
>
> ```tsx
> // src/components/ThemeRegistry.tsx  ← 'use client' が必要
> 'use client'
...
> **`.babelrc` について:**
>
> Next.js 15 は SWC コンパイラを使用しており、`.babelrc` を配置すると SWC が無効化されてビルドが遅くなる。
> emotion の設定は `next.config.ts` の `compiler.emotion: true` オプションで行うこと。
>
> ```ts
> // next.config.ts
> const nextConfig = {
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__b938c3c9d740b" frameborder="0"></iframe>
...
一度nextサーバーを再起動して、 [http://localhost:8000/hello\_mui](http://localhost:8000/hello_mui) にアクセスします。
...
![](https://static.zenn.studio/user-upload/d2ed3b1e2215-20230628.png)
...
![](https://static.zenn.studio/user-upload/6a940c74eb7a-20230628.png)

```
<Button variant="outlined">Hello Mui@v5!</Button>
```

![](https://static.zenn.studio/user-upload/dbd2bf2bd196-20230628.png)
...
![](https://static.zenn.studio/user-upload/28dea25fbfec-20230628.png)
...
![](https://static.zenn.studio/user-upload/08e83400beca-20230806.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__03c33efc55872" frameborder="0"></iframe>

つまり `color: { xs: 'white', md: 'red' }` は、ウィンドウサイズが0~899pxの範囲では `color: "white"` 、900px~の範囲では `color: "red"` として振る舞います。

![](https://static.zenn.studio/user-upload/10d098b02039-20230806.png)

![](https://static.zenn.studio/user-upload/1443cf74aa18-20230806.png)
...
![](https://static.zenn.studio/user-upload/5adf9b9975d6-20230628.png)
