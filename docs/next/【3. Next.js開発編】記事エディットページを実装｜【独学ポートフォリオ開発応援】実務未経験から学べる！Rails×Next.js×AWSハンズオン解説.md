---
title: "【3. Next.js開発編】記事エディットページを実装｜【独学ポートフォリオ開発応援】実務未経験から学べる！Rails×Next.js×AWSハンズオン解説"
source: "https://zenn.dev/ddpmntcpbr/books/rna-hands-on/viewer/next-11"
author:
published:
created: 2026-04-29
description:
...
> **[現バージョン対応メモ]**
>
> **ページファイルのパスと `'use client'`:**
>
> | 教材（Pages Router） | App Router（本プロジェクト） |
> |--------------------|--------------------------|
> | `src/pages/current/articles/edit/[id].tsx` | `src/app/current/articles/[id]/edit/page.tsx` |
>
> エディット画面はフォーム操作（`useState` / `onChange` / `onSubmit`）があるため `'use client'` が必要。
>
> **動的ルートパラメータ:**
>
> クライアントコンポーネントでは `useParams()` を使ってIDを取得する。
>
> ```ts
...
![](https://static.zenn.studio/user-upload/8ea226ba405d-20230811.png)

「プレビュー表示」のトグルスイッチを切り替えることで、フォームに入力された値を用いてリアルタイムにプレビュー画面を開くことができます。プレビュー画面では、本文中のマークダウンが展開されて表示されます。

![](https://static.zenn.studio/user-upload/e758c33c9d40-20230811.png)
...
![](https://static.zenn.studio/user-upload/3bd8b5dbd1a9-20230811.png)
...
![](https://static.zenn.studio/user-upload/0f243bf31768-20230811.png)
...
![](https://static.zenn.studio/user-upload/2111ff581627-20230811.png)

画面左上の左矢印アイコンをクリックすると、マイ記事一覧画面に戻ります。この画面では、先の未保存ステータスの記事は表示されていません。

![](https://static.zenn.studio/user-upload/09b8d400ba7b-20230811.png)
...
![](https://static.zenn.studio/user-upload/c18db3aa85f3-20230812.png)

タイトルを入力して「更新する」ボタンを押すと、「記事を保存しました」という通知が表示されます。

![](https://static.zenn.studio/user-upload/c4d7eb24c256-20230812.png)

一度、マイ記事一覧に戻ってみると、先ほどの記事が「下書き」として保存されていることが分かります。

![](https://static.zenn.studio/user-upload/3dd29cf44736-20230812.png)
...
![](https://static.zenn.studio/user-upload/e4bfbc56c0f4-20230812.png)
...
![](https://static.zenn.studio/user-upload/6c398a5b8827-20230812.png)

![](https://static.zenn.studio/user-upload/32e8268764ea-20230812.png)

タイトル、本文を入力し、「下書き／公開」のトグルをONにした状態で「更新する」を押すと、公開中ステータスとして記事を保存することができます。

![](https://static.zenn.studio/user-upload/e9694e1b7df1-20230812.png)

マイ記事一覧に戻ると、ステータスが「下書き」から「公開中」に切り替わっています。

![](https://static.zenn.studio/user-upload/13aea855dedb-20230812.png)

また、トップページ（ `/` ）に戻ると、先ほどの記事が全体の公開されており、記事詳細画面にアクセスできるようになっています。

![](https://static.zenn.studio/user-upload/d8496d1c5882-20230812.png)

![](https://static.zenn.studio/user-upload/24a3ff05b27f-20230812.png)
...
![](https://static.zenn.studio/user-upload/7581ecff2a81-20230905.png)
...
![](https://static.zenn.studio/user-upload/df9330f89f22-20230905.png)
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__a416c0f0e1851" frameborder="0"></iframe>
...
<iframe src="https://embed.zenn.studio/card#zenn-embedded__2467543f8049d" frameborder="0"></iframe>
...
![](https://static.zenn.studio/user-upload/678a53e8055d-20230812.png)
